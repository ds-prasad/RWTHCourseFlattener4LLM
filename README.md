...
1. **Most knowledge-base uploaders only accept files, not folders.**
   Uploading loses the hierarchy entirely because of which you end up with a flat pile of files
...
- **Safe cleanup:** extraction happens in a private temp folder that is deleted
  afterward as the script only removes files *it* created this run, never folders
  you already had.
...
- **Meant to be run ONCE per folder because no duplicate/idempotency checks.** Heads up,
